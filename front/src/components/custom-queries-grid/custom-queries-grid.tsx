...
const CustomQueriesGrid = () => {
    ...
    const onAddQueryBtnClick = () => {
        openModal('Add New Query', <NewQueryForm onSuccessSubmit={closeModal} />);
    }
                    ...
                    queries
                        ? queries.map(q =>
                            <Col span={12} key={q.id}>
                                <CustomQueryWrapper title={q.title} queryId={q.id!} />
                            </Col>)
                        : <h3 style={{ width: '100%', textAlign: 'center' }}>Create a new query with the button</h3>
                }
            </Row>
